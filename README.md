# 🦴 颈椎锻炼 · Neck Exercise Timer（AI 生成）

> 温和的颈椎锻炼计时器 —— 专为久坐人群设计的等长收缩训练助手，跟着语音节奏，每天几分钟，给颈椎做一套完整的"拉伸操"。

![Platform](https://img.shields.io/badge/Platform-Android-blue) ![Version](https://img.shields.io/badge/version-1.2(3)-green) ![minSdk](https://img.shields.io/badge/minSdk-26%20(Android%208.0)-orange) ![Deps](https://img.shields.io/badge/dependencies-zero-success) ![Tests](https://img.shields.io/badge/unit%20tests-48-brightgreen)
...
- **语音引导**：开始/换手/放松/阶段切换/完成全程中文 TTS 播报，不用盯屏幕
...
- 打卡历史本地持久化，重启不丢失（紧凑 `base36` 序列化，一条字符串存全部历史）
...
├── CheckIn.kt          # 打卡日志：TreeSet<epochDay> + v1:base36 紧凑序列化
...
- **打卡一条字符串**：全部历史序列化为 `v1:fz7,fz8,...`（base36 epochDay），读写一次搞定

更完整的规格说明（状态机、语音播报表、时长配置 schema）见 [`CLAUDE.md`](CLAUDE.md)；功能开发过程记录见 [`docs/compose/spec/`](docs/compose/spec/)。

## 🧪 测试

48 个 JUnit 单元测试（`app/src/test/`）覆盖：状态机全流程（舒缓播报序列逐字断言/等长三阶段切换/暂停恢复/计时抖动）、打卡逻辑（去重/跨月/streak 宽限规则/序列化容错）、提醒策略（跨天滚动/已打卡跳过）、JSON 解析往返。
